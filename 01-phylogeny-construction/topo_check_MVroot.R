...
get_btsp <- function(rawtree,tdf,cutoff,genomes)
{
  ...
  #no need to check if the btsp of each clade is > 60, since it is known 
  #most of the SNPs came from between-clade comparison; here we only focus on
...
setwd("E:/by28strains/MV_results")
# read *nwk file and lables of the tree
tree_dir <- "E:/by28strains/MV_results"
...
for(i in seq.int(TREE))#seq.int(TREE[subTREE]))#
{
  ...
  topo.type <- scan_ancestor_for_topo(rawtree,tree.df,genomes)
